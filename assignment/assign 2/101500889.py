...
Full Name: Perminder Kaur
Student Number: 101500889
...
def task1(total_salary):
    ...
    # Start writing code
    ########################
    gross_salary = int(total_salary) if isinstance(total_salary, str) else total_salary
    thresholds = [55000, 111000, 173000, 246000]
    tax_rates = ['15%', '21%', '26%', '29%', '33%']
    if gross_salary <= thresholds[0]:
        tax_rate = tax_rates[0]
        net_salary = gross_salary * (1 - 0.15)
    elif gross_salary <= thresholds[1]:
        tax_rate = tax_rates[1]
        net_salary = gross_salary * (1 - 0.21)
    elif gross_salary <= thresholds[2]:
        tax_rate = tax_rates[2]
        net_salary = gross_salary * (1 - 0.26)
    elif gross_salary <= thresholds[3]:
...
def task2(month_budget, total_expense, expense):
    """
    Code a function that accepts
        1) A monthly budget value
        2) A maximum expense value
        3) A dictionary of expenses with the following keys-value pair
    ...
    NOTE: you are required to add the necessary parameters, method body, and return statement.
    NOTE: error handling is not expected of you, assume that the values passed will be int, int and dictionary
    ...
    # Start writing code
    ########################
    total_expense_sum = sum(expense.values())

    for expense_name, expense_value in expense.items():
        if expense_value > total_expense:
            return (False, expense_name, expense_value)

    if total_expense_sum <= month_budget:
        return (True, total_expense_sum)

    return (False, total_expense_sum)
    pass
    ########################
    # End writing code
    ########################
...
def task3(variable):
    ...
    if isinstance(variable, int):
        return 'even' if variable % 2 == 0 else 'odd'
    elif isinstance(variable, str):
        return 'even' if len(variable) % 2 == 0 else 'odd'
    elif isinstance(variable, (list, set, tuple)):
        return 'even' if len(variable) % 2 == 0 else 'odd'
    else:
        return None
