...
Full Name: Abdulmannan Vahora
Student Number: 101546839
...
def task1():
    ...
    def task1(gross_salary):
        ...
        # Convert gross_salary to integer if it's a string
        if isinstance(gross_salary, str):
            gross_salary = int(gross_salary)

        # Determine tax rate and calculate net salary
        for rate, threshold in tax_brackets:
            if gross_salary <= threshold:
                tax_rate = f"{int(rate * 100)}%"
                net_salary = int(gross_salary * (1 - rate))
                break
...
def task3():
    ...
    def task3(input_value):
        if isinstance(input_value, int):
            return 'even' if input_value % 2 == 0 else 'odd'
        elif isinstance(input_value, str):
            return 'even' if len(input_value) % 2 == 0 else 'odd'
